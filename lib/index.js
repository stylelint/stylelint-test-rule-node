...
/**
 * @typedef {import('stylelint').PublicApi['lint']} Lint
 * @returns {Promise<Lint>}
 */
async function loadLint() {
	// @ts-expect-error -- TS2339: Property 'lint' does not exist on type 'typeof import("...")'.
	return (await import('stylelint')).default.lint; // eslint-disable-line n/no-unpublished-import
}
		...
		setupTestCases({
			...
			comparisons: (testCase) => async () => {
				...
				for (const [i, expected] of expectedWarnings.entries()) {
					assert.ok(expected.message, 'Expected "reject" test case to have a "message" property');

					const actualWarning = actualWarnings[i];

					assert.ok(actualWarning);
